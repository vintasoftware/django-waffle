...
class WaffleFlagViewMixin(object):
    ...
    def get_flag_name(self):
        assert (
            hasattr(self, 'get_flag_name') and self.get_flag_name is not None
        ), (
...
class WaffleSwitchViewMixin(object):
    ...
    def get_switch_name(self):
        assert (
            hasattr(self, 'get_switch_name') and 
                self.get_switch_name is not None
        ), (
            "'%s' should either include `flag_name` attribute, or "
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if type(redirect_to).__name__ == 'function':
            redirect_to = staticmethod(redirect_to)
